...
/**
 * Will remove a service from npwcore-node.json
...
/**
 * Will remove the Node.js service and from the npwcore-node configuration.
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The npwcore-node configuration directory
...
function remove(options, done) {
  ...
  var npwcoreConfigPath = path.resolve(configPath, 'npwcore-node.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(npwcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a npwcore-node.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      removeService(configPath, service, function(err) {
        ...
        // remove service to npwcore-node.json
        removeConfig(npwcoreConfigPath, service, next);
      });
    }, done
  );
}
