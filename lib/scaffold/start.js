...
function lookInModuleManifest(req, service) {
  try {
    ...
    if (servicePackage.npwcoreNode) {
      serviceModule = serviceModule + '/' + servicePackage.npwcoreNode;
...
function loadModule(req, service) {
  ...
  //package.json file, if so, then see if there is a npwcoreNode field, if so
  ...
  if (!serviceCode) {
    throw new Error('Attempted to load the ' + service.name + ' service from: ' +
      ...
      process.cwd() + '/package.json" - npwcoreNode field. All paths failed to find valid nodeJS code.');
  }
