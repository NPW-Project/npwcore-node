...
var BASE_PACKAGE = {
  description: 'A full NewPowerCoin node build with npwcore',
  repository: 'https://github.com/user/project',
  license: 'MIT',
  readme: 'README.md',
  dependencies: {
    'npwcore-lib': '^' + npwcore.version,
    'npwcore-node': version
  }
};

/**
 * Will create a directory for NPW.
 * @param {String} dataDir - The absolute path
 * @param {Function} done - The callback function called when finished
 */
function createNpwDirectory(datadir, done) {
...
/**
 * Will create a base NewPowerCoin Node configuration directory and files.
 * @param {Object} options
 * @param {String} options.network - "testnet" or "livenet"
 * @param {String} options.datadir - The npw database directory
...
function createConfigDirectory(options, configDir, isGlobal, done) {
  mkdirp(configDir, function(err) {
    ...
    try {
      fs.writeFileSync(configDir + '/npwcore-node.json', configJSON);
      if (!isGlobal) {
...
/**
 * Will setup a directory with a npwcore Node directory, configuration file,
 * npw configuration, and will install all necessary dependencies.
 *
 * @param {Object} options
 * @param {String} options.cwd - The current working directory
 * @param {String} options.dirname - The name of the npwcore node configuration directory
 * @param {String} options.datadir - The path to the npw datadir
...
function create(options, done) {
  ...
  async.series([
    function(next) {
      // Setup the the npwcore-node directory and configuration
    ...
    function(next) {
      // Setup the npw directory and configuration
      if (!fs.existsSync(absDataDir)) {
        createNpwDirectory(absDataDir, next);
      } else {
        next();
      }
    },
