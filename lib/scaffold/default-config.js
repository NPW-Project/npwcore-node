...
/**
 * Will return the path and default npwcore-node configuration. It will search for the
 * configuration file in the "~/.npwcore" directory, and if it doesn't exist, it will create one
...
function getDefaultConfig(options) {
  ...
  var defaultPath = path.resolve(process.env.HOME, './.npwcore');
  var defaultConfigFile = path.resolve(defaultPath, './npwcore-node.json');
  ...
  if (fs.existsSync(defaultConfigFile)) {
    ...
    console.log(`The configuration file at '${defaultConfigFile}' is incompatible with this version of npwcore.`);

    var now = new Date();
    // npwcore-node.YYYY-MM-DD.UnixTimestamp.json
    var backupFileName = `npwcore-node.${now.getUTCFullYear()}-${now.getUTCMonth()}-${now.getUTCDate()}.${now.getTime()}.json`;
  ...
  var defaultConfig = {
    ...
    datadir: options.datadir ? options.datadir : defaultDataDir,
    servicesConfig: {
      'insight-api': {
