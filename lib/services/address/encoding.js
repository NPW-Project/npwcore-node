...
// utxo type: 0-Ordinary, 1-PoW, 2-PoS, 3-MN
Encoding.prototype.encodeUtxoIndexValue = function(height, satoshis, timestamp, scriptBuffer, type) {
  var heightBuffer = new Buffer(4);
  ...
  var timestampBuffer = new Buffer(4);
  timestampBuffer.writeUInt32BE(timestamp || 0);
  var utxoTypeBuffer = new Buffer(1);
  utxoTypeBuffer.writeUInt8(type);
  return Buffer.concat([heightBuffer, satoshisBuffer, timestampBuffer, utxoTypeBuffer, scriptBuffer]);
};
...
Encoding.prototype.decodeUtxoIndexValue = function(buffer) {
  ...
  var scriptBuffer = buffer.slice(17);
  return {
    height: height,
    satoshis: satoshis,
    timestamp: timestamp,
