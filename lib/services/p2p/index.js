...
var P2P = function(options) {
  ...
  this._latestBits = 0x1e0ffff0;
  this._outgoingTxs = LRU(1000); // these are outgoing txs that are awaiting getdata messages
...
P2P.prototype._applyMempoolFilter = function(message) {
  ...
  var txIndex = this._mempoolFilter.indexOf(message.transaction.id);
  if (txIndex >= 0) {
...
P2P.prototype._onPeerBlock = function(peer, message) {
  this._blockCache.set(message.block.header.hash, message.block);
  this.emit(message.block.header.hash, message.block);
...
P2P.prototype._onPeerGetData = function(peer, message) {
  ...
  message.inventory.forEach(function(inv) {
    if (inv.type == p2p.Inventory.TYPE.TX) {
      var txId  = inv.hash.reverse().toString('hex');
      var tx = this._outgoingTxs.get(txId);
      if (tx) {
        peer.sendMessage(this.messages.Transaction(tx));
      }      
    }
  });
};
