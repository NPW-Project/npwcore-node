...
MempoolService.prototype.onReorg = function(args, callback) {
  ...
  for(var i = 0; i < oldBlockList.length; i++) {
    ...
    for(var j = 0; j < block.transactions.length; j++) {
      ...
      var key = this._encoding.encodeMempoolTransactionKey(tx.hash);
...
MempoolService.prototype.onBlock = function(block, callback) {
  ...
  for(var i = 0; i < block.transactions.length; i++) {
    ...
    ops.push({
      type: 'del',
      key: self._encoding.encodeMempoolTransactionKey(tx.hash)
    });
...
MempoolService.prototype._getAddressOperations = function(tx, reverse) {
  ...
  for(var i = 0; i < tx.outputs.length; i++) {
    var output = npwcore.Transaction.Output(tx.outputs[i]);
  ...
  for(i = 0; i < tx.inputs.length; i++) {
    var input = npwcore.Transaction.Input(tx.inputs[i]);
