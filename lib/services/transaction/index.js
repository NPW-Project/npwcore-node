    ...
    async.parallel([
      ...
      function(next) {
        async.eachOfLimit(tx.inputs, 4, function(input, index, next) {
          self._db.get(self._encoding.encodeDoubleSpentKey(input.prevTxId, index), function(err, value) {
            if (err) {
              return next(err);
            }
            if (!value) {
...
TransactionService.prototype.setTxMetaInfo = function(tx, options, callback) {
  ...
  async.waterfall([
    function(next) {
      if (tx.__inputValues && tx.__inputScripts) {
        return next(null, tx);
      }
      ...
      self._getInputValuesAndScripts(tx, options, function(err, inputValues, inputScripts) {
        if (err) {
          return callback(err);
        }

        tx.__inputValues = inputValues;
        tx.__inputScripts = inputScripts;
        next(null, tx);
      });
    },
    function(tx, next) {
      ...
      if (_tx.isCoinbase()) {
        ...
        tx.feeSatoshis = 0;
        tx.isCoinbase = true;
        tx.rewardSatoshis = tx.outputSatoshis;
      } else {
        var inputSatoshis = 0;
        ...
        if (_tx.isCoinStake()) {
          tx.feeSatoshis = 0;
          tx.isCoinStake = true;
          tx.rewardSatoshis = outputSatoshis - inputSatoshis;
          tx.mnRewardSatoshis = tx.outputs[tx.outputs.length-1].satoshis;
          tx.posRewardSatoshis = tx.rewardSatoshis - tx.mnRewardSatoshis;
        } else {
...
TransactionService.prototype._getInputValuesAndScripts = function(tx, options, callback) {
  var self = this;
  ...
  async.mapLimit(tx.inputs, 4, function(input, next) {
    if (npwcore.Transaction.Input(input).isNull()) {
      return next(null, {val: 0, script: input.script});
    }
    ...
    async.waterfall([
      ...
      function(next) {
        self._getTransaction(input.prevTxId, options, next);
      },
      ...
      function(txid, tx, next) {
        if (!tx) {
          ...
          options.block.transactions.forEach(function (tx1) {
        ...
        if (!tx) {
          return next(new Error('Transaction Service: prev transacion: (' + input.prevTxId + ') for tx: ' +
            _tx.hash + ' at input index: ' + outputIndex + ' is missing from the index or not in the memory pool. It could  be' +
        ...
        next(null, output.satoshis, output.script);
      }
    ], function(err, val, script) {
      if (err) {
        return next(err);
      }
      next(null, {val: val, script: script});
    });
  }, function(err, vals) {
    if (err) {
      callback(err);
    ...
    vals.forEach(function (val) {
      inputValues.push(val.val);
      inputScripts.push(val.script);
    });
    callback(null, inputValues, inputScripts);
  });
};
...
TransactionService.prototype.onBlock = function(block, callback) {
  ...
  async.mapSeries(block.transactions, function(tx, next) {
    ...
    assert(block.transactions.length === operations.length, 'It seems we are not indexing the correct number of transactions.');
...
TransactionService.prototype.onReorg = function(args, callback) {
  ...
  for(var i = 0; i < oldBlockList.length; i++) {
    ...
    for(var j = 0; j < block.transactions.length; j++) {
      ...
      removalOps.push({
        type: 'del',
        key: self._encoding.encodeTransactionKey(tx.hash)
      });
      ...
      for(var k = 0; k < tx.inputs.length; k++) {
        ...
        removalOps.push({
          type: 'del',
          key: self._encoding.encodeSpentKey(input.prevTxId, input.outputIndex)
        });
      }
    }
  }
...
TransactionService.prototype._getSpentInfo = function(input, callback) {
  if (!this.node.stopping) {
    return this._db.get(this._encoding.encodeSpentKey(input.prevTxId, input.outputIndex), callback);
  }
  callback();
};
...
TransactionService.prototype._getSpentTxOperations = function(tx, callback) {
  ...
  async.eachOfLimit(tx.inputs, 4, function(input, index, next) {
    self._getSpentInfo(input, function(err, info) {
      ...
      if (info) {
        ops.push({
          key: self._encoding.encodeDoubleSpentKey(input.prevTxId, input.outputIndex),
          value: self._encoding.encodeDoubleSpentValue(tx.hash, index, tx.__height, tx.__blockhash)
        });
        return next();
      }
      ...
      ops.push({
        key: self._encoding.encodeSpentKey(input.prevTxId, input.outputIndex),
        value: self._encoding.encodeSpentValue(tx.hash, index, tx.__height, tx.__blockhash)
      });
      next();
    });
  }, function(err) {
...
TransactionService.prototype._processTransaction = function(tx, opts, callback) {
  ...
  self._getInputValuesAndScripts(tx, opts, function(err, inputValues, inputScripts) {
    ...
    // timestamp
    tx.__timestamp = self._getBlockTimestamp(opts.block.header.hash);
    ...
    // block hash
    tx.__blockhash = opts.block.header.hash;
