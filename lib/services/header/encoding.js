...
Encoding.prototype.encodeHeaderValue = function(header) {
  ...
  var headerValue = Buffer.concat([hashBuf, versionBuf, prevHash, merkleRoot, tsBuf,
    bitsBuf, nonceBuf, heightBuf, chainworkBuf, nextHash]);

  if (header.version >= 3) {
    var accumulatorCheckpoint = new Buffer(header.accumulatorCheckpoint, 'hex');
    headerValue = Buffer.concat([hashBuf, versionBuf, prevHash, merkleRoot, tsBuf,
      bitsBuf, nonceBuf, accumulatorCheckpoint, heightBuf, chainworkBuf, nextHash]);
  }
...
Encoding.prototype.decodeHeaderValue = function(buffer) {
  ...
  if (version >=3) {
    var accumulatorCheckpoint = buffer.slice(112, 144).toString('hex');
    var height = buffer.readUInt32BE(144);
    var chainwork = buffer.slice(148, 148 + 32).toString('hex');
    var nextHash = buffer.slice(148 + 32).toString('hex');
    return {
      hash: hash,
      version: version,
      prevHash: prevHash,
