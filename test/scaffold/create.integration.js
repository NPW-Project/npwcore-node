...
describe('#create', function() {
  ...
  before(function(done) {
    ...
    mkdirp(testDir, function(err) {
      ...
      mkdirp(testDir + '/.npwcore', function(err) {
        if (err) {
          throw err;
        }
        done();
  ...
  it('will create scaffold files', function() {
    create({
      ...
      var configPath = testDir + '/mynode/npwcore-node.json';
      ...
      config.services.should.deep.equal(['address', 'block', 'db', 'fee', 'header', 'mempool', 'p2p', 'timestamp', 'transaction', 'web']);
      config.datadir.should.equal(path.resolve(testDir, './mynode/data'));
